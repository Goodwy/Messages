...
class ThreadActivity : SimpleActivity() {
    ...
    override fun onResume() {
        ...
        if (config.threadTopStyle == THREAD_TOP_LARGE) {
            top_details_compact.beGone()
            setupToolbar(thread_toolbar, NavigationIcon.Arrow, toolbarColor, appBarLayout = thread_app_bar_layout)
            updateStatusbarColor(getBottomNavigationBackgroundColor())
            thread_app_bar_layout.setBackgroundColor(getBottomNavigationBackgroundColor())
        } else {
            top_details_large.beGone()
            setupToolbar(thread_toolbar, NavigationIcon.Arrow, appBarLayout = thread_app_bar_layout)
        }
    ...
    private fun setupAdapter() {
        ...
        confirm_inserted_number.setColorFilter(getProperTextColor())

        add_contact_or_number.setBackgroundResource(R.drawable.search_bg)
        add_contact_or_number.backgroundTintList = ColorStateList.valueOf(getBottomNavigationBackgroundColor())
    }
    ...
    private fun setupButtons() {
        ...
        thread_add_attachment.background.applyColorFilter(getBottomNavigationBackgroundColor())
        thread_type_message_holder.background.applyColorFilter(getBottomNavigationBackgroundColor())
        ...
        thread_character_counter.backgroundTintList = getProperBackgroundColor().getColorStateList()
        ...
        thread_send_message_holder.setOnClickListener {
            sendMessage()
        }

        thread_send_message_holder.isClickable = false
        thread_send_message.backgroundTintList = getProperPrimaryColor().getColorStateList()
        ...
        thread_add_attachment_holder.setOnClickListener {
        ...
        when (config.threadTopStyle) {
            THREAD_TOP_COMPACT -> {
                if (threadTitle.isNotEmpty()) {
                    sender_name.text = threadTitle
                    sender_name.setTextColor(getProperTextColor())
                }
                sender_number.beGoneIf(threadTitle == threadSubtitle || participants.size > 1)
                sender_number.text = threadSubtitle
                sender_number.setTextColor(getProperTextColor())
            }
            THREAD_TOP_LARGE -> {
                top_details_compact.beGone()
                if (threadTitle.isNotEmpty()) {
                    sender_name_large.text = threadTitle
                    sender_name_large.setTextColor(getProperTextColor())
                }
                sender_number_large.beGoneIf(threadTitle == threadSubtitle || participants.size > 1)
                sender_number_large.text = threadSubtitle
                sender_number_large.setTextColor(getProperTextColor())
            }
        }
    }
    ...
    private fun setupSIMSelector() {
        ...
        if (availableSIMs.size > 1) {
            ...
            thread_select_sim_icon.background.applyColorFilter(getBottomNavigationBackgroundColor())
    ...
    private fun showSelectedContacts() {
        ...
        participants.forEach { contact ->
            layoutInflater.inflate(R.layout.item_selected_contact, null).apply {
                val selectedContactBg = resources.getDrawable(R.drawable.item_selected_contact_background)//item_selected_contact_background
                ...
                selected_contact_remove.beGoneIf(contact.rawId == participants.first().rawId)
    ...
    private fun checkSendMessageAvailability() {
        if (thread_type_message.text!!.isNotEmpty() || (attachmentSelections.isNotEmpty() && !attachmentSelections.values.any { it.isPending })) {
            thread_send_message_holder.isClickable = true
    ...
    private fun updateContactImage() {
        val senderPhoto = when (config.threadTopStyle) {
            THREAD_TOP_COMPACT -> sender_photo
            THREAD_TOP_LARGE -> sender_photo_large
            else -> sender_photo
        }
        val placeholder = if (participants.size > 1) {
            SimpleContactsHelper(this).getColoredGroupIcon(participants.getThreadTitle())
        } else {
            null
        }
        if (participants.firstOrNull() == null) {
            if (!isDestroyed || !isFinishing) {
                SimpleContactsHelper(this).loadContactImage("", senderPhoto, participants.getThreadTitle(), placeholder)
            }
        } else {
            val phoneNumber = participants.firstOrNull()?.phoneNumbers?.firstOrNull()?.normalizedNumber ?: return
            if (participants.first().name == phoneNumber) {
                val drawable = resources.getDrawable(R.drawable.placeholder_contact)
                if (baseConfig.useColoredContacts) {
                    val color = letterBackgroundColors[Math.abs(participants.getThreadTitle().hashCode()) % letterBackgroundColors.size].toInt()
                ...
                if (!isDestroyed || !isFinishing) {
                    SimpleContactsHelper(this).loadContactImage(participants.first().photoUri, senderPhoto, participants.getThreadTitle(), placeholder)
                }
            }
        }
